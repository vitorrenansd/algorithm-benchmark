...
public class ImageExtractor {
     ...
     // Method to get the last modified timestamps of files
        public void getFileLastModified(String directoryName) {
            ...
            if (files != null) {
                // Collect the last modified timestamps of each file
                for (File file : files) {
                    lastModifiedList.add(file.lastModified()); // Returns UNIX timestamp
                }
    
                // Print the results automatically
                System.out.println("\nLast Modifications (UNIX timestamps):");
                for (Long timestamp : lastModifiedList) {
                    System.out.println(sfd.format(timestamp));
                }
            } else {
